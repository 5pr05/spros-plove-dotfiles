...
function buildCarousel(containerId, selectN, defaultIdx) {
    ...
    wallpapers.forEach((w, idx) => {
        ...
        `;
        card.onclick = () => {
            document.querySelectorAll(`#${containerId} .wall-card`).forEach(c => c.classList.remove('active'));
            card.classList.add('active');
            if (selectN === 1) selW1 = w; else selW2 = w;
        };
...
function updateUI() {
    ...
    document.getElementById('label-mod4').classList.toggle('active', mod === 'Mod4');
    document.getElementById('label-mod1').classList.toggle('active', mod === 'Mod1');
    ...
    const m1 = document.getElementById('mon_left').value || 'eDP-1';
    const m2 = document.getElementById('mon_right').value || 'HDMI-A-2';
    document.getElementById('mon-code').textContent = dual ? `${m1}  ${m2}` : m1;
}

function copyCmd(btn) {
    navigator.clipboard.writeText('swaymsg -t get_outputs | grep name').then(() => {
...
function syncColor(id) {
    ...
    const keyMap = { c_primary: 'primary', c_secondary: 'secondary', c_tertiary: 'tertiary', c_accent: 'accent' };
...
set $monitor-left   ${document.getElementById('mon_left').value}
set $monitor-right  ${isDual ? document.getElementById('mon_right').value : 'NONE'}
